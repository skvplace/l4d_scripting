...
void OnPressed(const char[] output, int button, int activator, float delay)
{
	...
	int trigger = GetButtonTrigger(button);
	if (trigger)
	{
...
void OnUseLocked(const char[] output, int button, int client, float delay)
{
	...
	{
		IsSurvivorsEntireTeam(client, true);
	}
	else
	{
...
bool IsSurvivorsEntireTeam(int client, bool message = false)
{
	if (!IsValidClientTeam2Alive(client))
	{
		return false;
...
int GetButtonTrigger(int button)
{
	if (!IsValidEntity(button))
	{
		return 0;
	...
	while ((i = FindEntityByClassname(i, "trigger_multiple")) != -1)
	{
		if (GetEntPropEnt(i, Prop_Data, "m_hOwnerEntity") == button)
		{
			return i;
		}
	}
...
Action:Teleport(Handle timer, int ref)
{
	...
	if (!strcmp(classname, "func_button"))
	{
		...
		int trigger = GetButtonTrigger(entity);
		if (trigger)
		{
			UnhookSingleEntityOutput(trigger, "OnEntireTeamStartTouch", OnEntireTeamStartTouch);
...
void Search_SafeDoor()
{
	...
	while ((i = FindEntityByClassname(i, "prop_door_rotating_checkpoint")) != -1)
	{
		...
		{
			if (!FindDistEntityByClassname(i, "player", 500.0))
			{
				HookSingleEntityOutput(i, "OnFullyClosed", OnFullyClosed);
	...
	int trigger = SafeRoomTrigger_Spawn(-1); 
	if (trigger)
	{
		HookSingleEntityOutput(trigger, "OnEntireTeamStartTouch", OnEntireTeamStartTouch);
