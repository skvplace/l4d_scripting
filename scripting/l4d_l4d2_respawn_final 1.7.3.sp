...
#define VISIBLE_HEIGH				80.0 // 50.0
...
public void Event_finale_start(Handle event, const char[] name, bool dontBroadcast)
{
	RTimerKill(gt_Check_FinaleStart);
	
	gt_Check_FinaleStart = RTimerCreate(5.0, Check_FinaleStart, _, TIMER_FLAG_NO_ROUNDCHANGE);
}

public void Event_finale_win(Handle event, const char[] name, bool dontBroadcast)
{
	Delete_Timers();
}

void Check_FinaleStart(Handle timer)
{
	SearchDeadSurvivor(null);
...
public void Event_survivor_rescued(Handle event, const char[] name, bool dontBroadcast)
{
	if (!gb_finale_start)
	{
		return;
	...
	if (!IsValidHandle(gt_SearchDeadSurvivor))
	{
		gt_SearchDeadSurvivor = RTimerCreate(1.0, SearchDeadSurvivor, _, TIMER_FLAG_NO_ROUNDCHANGE);
	}
}
...
public void Event_player_team(Handle event, const char[] name, bool dontBroadcast)
{
	...
	gt_CheckClientDead[survivorid] = RTimerCreate(gf_rescue_min_dead_time_final, CheckClientDead, survivorid, TIMER_REPEAT | TIMER_FLAG_NO_ROUNDCHANGE);
...
public void Event_player_death(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (survivorid)
	{
		...
		RTimerKill(gt_CheckClientDead[survivorid]);
...
Action CheckClientDead(Handle timer, int survivorid)
{
	...
	gt_CreateRescue[survivorid] = RTimerCreate(3.0, CreateRescue, survivorid, TIMER_REPEAT | TIMER_FLAG_NO_ROUNDCHANGE);
	
	return Plugin_Stop;
}

public void Event_entity_visible(Handle event, const char[] name, bool dontBroadcast)
{
	if (!gb_finale_start)
	{
		return;
...
public void Event_player_use(Handle event, const char[] name, bool dontBroadcast)
{
	if (!gb_finale_start)
	{
		return;
...
bool OpenDoor(int rescue, int rescuer)
{
	...
	if (!rescuer || !IsEntityValid(rescuer)) 
	{
		return false;
	}
...
Action Rescue_Playsound_Timer(Handle timer, Handle h_data)
{
	...
	if (respawn_outside_time)
	{
		if (RoundFloat(GetGameTime()) - gi_time_playerid[survivorid] >= respawn_outside_time)
		{
			...
			gt_CheckClientDead[survivorid] = RTimerCreate(yell_interval, CheckClientDead, survivorid, TIMER_REPEAT | TIMER_FLAG_NO_ROUNDCHANGE);
...
void Delete_Timers()
{
	...
	for (int i = 1; i <= MAX_SURVIVORID; i++)
	{
		RTimerKill(gt_CheckClientDead[i]);
...
void SetOriginDeadSurvivors(int survivorid)
{
	...
	if (strcmp(gs_name_clone, "empty"))
	{
		...
		if (time > 0.0)
		{
			RTimerCreate(time, SetOriginCloneSurvivor, survivorid, TIMER_FLAG_NO_ROUNDCHANGE);
		}
	}
}

void SetOriginCloneSurvivor(Handle timer, int survivorid)
{
	if (!survivorid)
	{
		return;
...
void SearchDeadSurvivor(Handle timer)
{
	if (!gi_respawn_mode_finale)
	{
		return;
	...
	for (int i = 1; i <= MAX_PLAYERS; i ++)
	{
		if (IsValidClientTeam2(i) && !IsPlayerAlive(i))
		{
			...
			if (survivorid)
			{
				...
				if (!IsValidHandle(gt_CheckClientDead[survivorid]))
				{
					gt_CheckClientDead[survivorid] = RTimerCreate(time_respawn, CheckClientDead, survivorid, TIMER_REPEAT | TIMER_FLAG_NO_ROUNDCHANGE);
				}
			}		
		}
	}
...
public void OnPluginReviveSurvivor(char[] clone_name)
{
