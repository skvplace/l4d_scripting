...
#define FILE_BUTTON_TRIGGER 				"addons/sourcemod/data/ts_button_triggers.txt"
#define FILE_ESCAPE_ENABLER 				"addons/sourcemod/data/ts_escape_enabler.txt"
...
int 	gi_countdown_escape_vehile;

Handle 	gk_button_triggers;
Handle 	gk_escape_enabler;
...
int 	gi_help_target						[MAX_SURVIVORID + 1]; // player
...
public void OnPluginStart()
{
	...
	SetConVarFlags(gc_teleport_countdown_escape_mode, 	GetConVarFlags(gc_teleport_countdown_escape_mode) & ~FCVAR_NOTIFY);
	...
	SetConVarFlags(gc_teleport_bot_rescue, 				GetConVarFlags(gc_teleport_bot_rescue) & ~FCVAR_NOTIFY);
...
void Events_SetHooks()
{
	...
	HookEvent("player_now_it", 				Event_player_attacked);
...
void Events_RemoveHooks()
{
	...
	UnhookEvent("player_now_it", 			Event_player_attacked);
	...
	UnhookEvent("survivor_call_for_help", 	Event_survivor_call_for_help);
...
public void OnGameplayStart(int stage)
{
	...
	if (GetConVarInt(gc_teleport_countdown_escape))
	{
		...
		int escape_mode = GetConVarInt(gc_teleport_countdown_escape_mode);
		...
		for (int i = 1; i <= MAX_ESCAPE_DATA; i++)
		{
			if (gi_escape_trigger_hammerid[i])
			{
				...
				if (IsEntityValid(entity))
				{
					...
					{
						Search_EscapeTriggers(entity);
						HookSingleEntityOutput(entity, "OnEntireTeamStartTouch", OnEntireTeamStartTouch_Escape);
					}
				}
			}
		}
...
void OnEntireTeamStartTouch_Escape(const char[] output, int trigger, int client, float delay)
{
	if (GetConVarInt(gc_teleport_countdown_escape_mode) == 2)
	{
		if (RTimerKill(gt_EscapeVehicle_Countdown))
		{
			PrintHintTextToAll(" ");
		}
...
void OnEnable_Escape(const char[] output, int entity, int activator, float delay)
{
	if (GetConVarInt(gc_teleport_countdown_escape_mode) == 2)
	{
		int trigger = GetEntPropEnt(entity, Prop_Data, "m_hOwnerEntity");
		
		char targetname[MAX_STRING_LENGTH]; 
		GetEntPropString(trigger, Prop_Data, "m_iName", targetname, sizeof(targetname));
		
		KvRewind(gk_escape_enabler);
		
		if (KvJumpToKey(gk_escape_enabler, targetname, false))
		{
			float time = KvGetFloat(gk_escape_enabler, "delay");
			
			KvRewind(gk_escape_enabler);
			
			gi_countdown_escape_vehile = GetConVarInt(gc_teleport_countdown_escape_vehile);
		
			gt_EscapeVehicle_Countdown = RTimerCreate(time, EscapeVehicle_Countdown, trigger, TIMER_REPEAT | TIMER_FLAG_NO_ROUNDCHANGE);
		}
	}
}

Action EscapeVehicle_Countdown(Handle timer, int owner)
{
	RTimerSetInterval(timer, 1.0);
	
	if (gi_countdown_escape_vehile > 0)
	{
		PrintHintTextToAll("%t: %d", "Vehicle Countdown", gi_countdown_escape_vehile);
		
		gi_countdown_escape_vehile --;
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		{
			if (!IsClientInTrigger(i, owner))
			{
				survivorid = GetClientSurvivorId(i);
				if (survivorid)
				{
					RTimerKill(gt_Force_PlayerSuicide[survivorid]);
					
					gt_Force_PlayerSuicide[survivorid] = RTimerCreate(5.0, Force_PlayerSuicide, survivorid, TIMER_FLAG_NO_ROUNDCHANGE);
				}
			}
			else
			{
	...
	{
		int i = -1;
		while ((i = FindEntityByClassname(i, "trigger_finale")) != -1)
		{
			DispatchKeyValue(i, "targetname", "disabled");
			InputEntity(i, "Disable");
		}
	}
	...
	{
		char targetname[MAX_STRING_LENGTH]; 
		GetEntPropString(owner, Prop_Data, "m_iName", targetname, sizeof(targetname));
		...
		if (KvJumpToKey(gk_escape_enabler, targetname, false))
		{
			...
			if (IsEntityValid(entity))
			{
				...
				int total = KvGetNum(gk_escape_enabler, "total", 0);
				...
				if (total)
				{
					for (int i = 1; i <= total; i++)
					{
						...
						KvGetString(gk_escape_enabler, num, value, sizeof(value), "empty");
						...
						{
							DispatchKeyValue(entity, "OnTrigger", value);
						}
					}
...
void Force_PlayerSuicide(Handle timer, int survivorid)
{
	if (gb_finale_win)
	{
		return;
	...
	ForcePlayerSuicide(GetClientOfSurvivorId(survivorid));
}
...
void DoorwayTrigger_Spawn(int safedoor)
{
	...
	HookSingleEntityOutput(entity, "OnTrigger", OnStartTouch_doorway);
}
...
void OnStartTouch_doorway(const char[] output, int trigger, int client, float delay)
{
	...
	UnhookSingleEntityOutput(trigger, "OnStartTouch", OnStartTouch_doorway);
	UnhookSingleEntityOutput(trigger, "OnTrigger", OnStartTouch_doorway);
...
bool IsVisible(int client, int entity)
{
	...
	MovePos_Forward(pos_start, ang_client, 5.0); // 10.0
	*/
...
void Search_EscapeTriggers(int trigger)
{
	if (gk_escape_enabler == null)
	{
		gk_escape_enabler = CreateKeyValues("escape_enabler");
		if (gk_escape_enabler == null)
		{
			return;
		}
		
		if (!FileToKeyValues(gk_escape_enabler, FILE_ESCAPE_ENABLER))
		{
			return;
		}
	}
	...
	char key_value	[PLATFORM_MAX_PATH];
	KvGetString(gk_escape_enabler, "hammerid", key_value, sizeof(key_value));
	...
	KvGetString(gk_escape_enabler, "output", output, sizeof(output));
	
	SetEntPropEnt(entity, Prop_Data, "m_hOwnerEntity", trigger);
	HookSingleEntityOutput(entity, output, OnEnable_Escape);
...
void Search_Trigger()
{
	...
	bool search_button_trigger;
	bool search_escape_enabler;
			...
			if (!strcmp(classname, "trigger_multiple"))
			{
				...
				if (entry.GetNextKey("OnEntireTeamStartTouch", keyvalue, sizeof(keyvalue)) != -1)
				{
					...
					if (unlock)
					{
						...
						search_button_trigger = Search_ButtonTrigger(hammerid, button_targetname);
					}
					else if (strlen(trigger_targetname)) 
					{
							...
							!strcmp(trigger_targetname, "trigger_escape"))
						{
							...
							if (Search_EnablerTrigger(trigger_targetname))
							{
								KvRewind(gk_escape_enabler);
		
								if (KvJumpToKey(gk_escape_enabler, trigger_targetname, false))
								{
									search_escape_enabler = true;
									...
									for (int j = 0; j < entry.Length; j++)
									{
										...
										if (!strcmp(key, "OnEntireTeamStartTouch"))
										{
											d ++;
											
											FormatEx(num, sizeof(num), "%d", d);
											KvSetString(gk_escape_enabler, num, keyvalue);
										}
									}
									
									KvSetNum(gk_escape_enabler, "total", d);
									KvRewind(gk_escape_enabler);
								}
							}
						}
	...
	if (search_button_trigger)
	{
		KeyValuesToFile(gk_button_triggers, FILE_BUTTON_TRIGGER);
	}
	
	if (search_escape_enabler)
	{
		KeyValuesToFile(gk_escape_enabler, FILE_ESCAPE_ENABLER);
	}
}

bool Search_EnablerTrigger(char trigger_targetname[PLATFORM_MAX_PATH])
{
	char hammerid	[PLATFORM_MAX_PATH];
	char key		[PLATFORM_MAX_PATH];
	char keyvalue	[PLATFORM_MAX_PATH];
	char name		[PLATFORM_MAX_PATH];
	char temp		[PLATFORM_MAX_PATH];
	char buffer		[PLATFORM_MAX_PATH];
	char new_value	[PLATFORM_MAX_PATH];
	...
	for (int i, n = EntityLump.Length(); i < n; i++)
	{
		...
		for (int j = 0; j < entry.Length; j++)
		{
			...
			SplitString(keyvalue, ",", name, sizeof(name));
			
			if (strcmp(key, "targetname") && 
				!strcmp(name, trigger_targetname) &&
				StrContains(keyvalue, "Enable") > -1)
			{
				entry.GetNextKey("hammerid", hammerid, sizeof(hammerid));
							
				KvRewind(gk_escape_enabler);
		
				if (KvJumpToKey(gk_escape_enabler, trigger_targetname, true))
				{
					KvSetString(gk_escape_enabler, "hammerid", hammerid);
					KvSetString(gk_escape_enabler, "output", key);
					
					if (GetEngineVersion() == Engine_Left4Dead)
					{
						FormatEx(temp, sizeof(temp), keyvalue);
						FormatEx(new_value, sizeof(new_value), "%s,Enable,,", trigger_targetname);
						
						len = strlen(new_value);
						strcopy(temp, sizeof(temp), temp[len]);
						
						SplitString(temp, ",", buffer, sizeof(buffer));
						
						KvSetFloat(gk_escape_enabler, "delay", StringToFloat(buffer));
					}
					else
					{
						FormatEx(temp, sizeof(temp), keyvalue);
...
bool Search_ButtonTrigger(int trigger_hammerid, char button_targetname[PLATFORM_MAX_PATH])
{
	char classname	[PLATFORM_MAX_PATH];
	char keyvalue	[PLATFORM_MAX_PATH];
...
void Event_survivor_call_for_help(Handle event, const char[] name, bool dontBroadcast)
{
	...
	float delay = GetConVarFloat(gc_teleport_bot_rescue);
	if (delay <= 0.0)
	{
	...
	if (RTimerIsValid(gt_TimerRescue[survivorid]))
	{
		return;
	}
...
int GetClientFromTimerRescue(int ref)
{
	for (int survivorid = 1; survivorid <= MAX_SURVIVORID; survivorid ++)
	{
		if (RTimerIsValid(gt_TimerRescue[survivorid]) && RTimerGetValue(gt_TimerRescue[survivorid]) == ref)
		{
...
int GetNearBot(int entity)
{
	...
	float pos_i[3];
	float near_distance; float distance;	
	...
	for (int i = 1; i <= MaxClients; i++)
	{
			...
			!IsSurvivorReviveTarget(i))
		{
			...
			{
				if (!near_distance || near_distance > distance)
				{
...
stock int IsSurvivorReviveTarget(int client)
{
	int m_reviveTarget = GetEntPropEnt(client, Prop_Send, "m_reviveTarget");
	if (m_reviveTarget > 0)
	{
		return m_reviveTarget;
	}
	
	return 0;
}

stock int IsSurvivorReviveOwner(int client)
{
	int m_reviveOwner = GetEntPropEnt(client, Prop_Send, "m_reviveOwner");
	if (m_reviveOwner > 0)
	{
		return m_reviveOwner;
	}
	
	return 0;
}

stock int IsSurvivorHealTarget(int client)
{
	if (GetEngineVersion() != Engine_Left4Dead)
	{
		return 0;
	...
	int m_healTarget = GetEntPropEnt(client, Prop_Send, "m_healTarget");
	if (m_healTarget > 0)
	{
		return m_healTarget;
	}
...
stock int IsSurvivorHealOwner(int client)
{
	if (GetEngineVersion() != Engine_Left4Dead)
	{
		return 0;
	...
	int m_healOwner = GetEntPropEnt(client, Prop_Send, "m_healOwner");
	if (m_healOwner > 0)
	{
		return m_healOwner;
	}
...
/**
 * Проверяет клиента команды 3
 *
 * client 			- Client index.
 * return 			- true если client valid и false если нет
 */
stock bool IsValidClientTeam3Alive(int client)
{
	if (IsValidClientAlive(client))
	{
...
stock bool IsPlayerTank(int client)
{
	if (IsValidClient(client)) 
	{
		if (GetEngineVersion() == Engine_Left4Dead && GetEntProp(client, Prop_Send, "m_zombieClass") == 5)
		{
			return true;
		}
		else if (GetEngineVersion() == Engine_Left4Dead2 && GetEntProp(client, Prop_Send, "m_zombieClass") == 8)
		{
			return true;
		}
	}
