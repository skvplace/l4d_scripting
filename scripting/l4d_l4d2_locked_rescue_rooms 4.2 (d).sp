...
bool ReCreateAllRescuesRoom(int room)
{
	...
	int amount = 1;
	FormatEx(temp, sizeof(temp), "rescue %d", amount);
...
void KeyValues_ToFile(Handle kv, char [] file_name)
{
	...
	FormatEx(buffer, sizeof(buffer), "addons/sourcemod/data/LR_%s.txt", file_name);
...
void RenameRescueDoor(int door, float distance = 120.0)
{
	...
	{
		char targetname[MAX_CLASSNAME_LENGTH];
		FormatEx(targetname, sizeof(targetname), "lr_door_renamed_%d", door);
