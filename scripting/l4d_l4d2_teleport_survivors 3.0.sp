...
float 	gi_survivor_trigger_touch			[MAX_SURVIVORID + 1];
...
void OnStartTouch(const char[] output, int trigger, int client, float delay)
{
	if (!IsValidClientTeam2Alive(client))
	{
		return;
	...
	ClientTrigger_Set(client, trigger);
}
...
void OnFullyClosed(char [] output, int safedoor, int client, float delay)
{
	...
	if (!gv_pos_opener[0] && !gv_pos_opener[1] && !gv_pos_opener[2])
	{
		...
		ang_entity[0] = 0.0;
		ang_entity[1] = GetAngleOrigin(pos_door, pos_entity);
		ang_entity[2] = 0.0;
...
void OnStartTouch_Saferoom(const char[] output, int trigger, int client, float delay)
{
	...
	if (OnTriggetEntireTeam(trigger))
	{
		return;
	}
	...
	int near_survivor = GetNearSurvivorGathered(trigger);
	if (!near_survivor)
	{
		return;
	}
...
int DoorwayTrigger_Spawn(int safedoor)
{
	if (!IsEntityValid(safedoor))
	{
		return 0;
...
void OnStartTouch_doorway(const char[] output, int entity, int client, float delay)
{
	if (!IsValidClientTeam2Alive(client))
	{
		return;
	...
	UnhookSingleEntityOutput(entity, output, OnStartTouch_doorway);
...
void OnEndTouch_Elevator(const char[] output, int trigger, int client, float delay)
{
	...
	DataPack pack;
	if (IsValidHandle(RTimerDataCreate(0.2, ClientTrigger_Remove, pack, TIMER_FLAG_NO_ROUNDCHANGE)))
	{
		WritePackCell(pack, GetClientSurvivorId(client));
...
void Delete_Timers()
{
	if (IsValidHandle(gt_Teleport))
	{
		CloseHandle(gt_Teleport);
	}
	
	if (IsValidHandle(gt_SearchFarBots))
	{
		CloseHandle(gt_SearchFarBots);
	}
	
	for (int i = 1; i <= MAX_SURVIVORID; i++)
	{
		gi_survivor_trigger			[i] = 0;
		gi_survivor_trigger_touch	[i] = 0.0;
		
		if (IsValidHandle(gt_TimerThreat[i]))
		{
			delete gt_TimerThreat[i];
		}
		
		if (IsValidHandle(gt_BotHelps[i]))
		{
			delete gt_BotHelps[i];
		}
...
void Teleport_Start(int entity, int countdown)
{
	...
	if (!GetConVarBool(gc_teleport_bot_countdown) && IsAllHumansInside(entity))
	{
		Teleport(null,  EntIndexToEntRef(entity));
		return;
	}
...
Action Teleport(Handle timer, int ref)
{
	...
	if (!strcmp(classname, "func_button"))
	{
		...
		InputEntity(entity, "Press"); // , _, 0.5
...
void Teleport_Cancel(Handle timer)
{
	if (IsValidHandle(gt_Teleport))
	{
		CloseHandle(gt_Teleport);
...
void Search_SafeDoor()
{
	...
	int trigger_doorway = DoorwayTrigger_Spawn(safedoor);
	if (trigger_doorway)
	{
		HookSingleEntityOutput(trigger_doorway, "OnStartTouch", OnStartTouch_doorway);
	}
}
...
void ClientTrigger_Set(int client, int trigger)
{
	...
	{
		gi_survivor_trigger_touch[survivorid] = GetGameTime();
...
void ClientTrigger_Remove(Handle timer, Handle data)
{
	...
	int survivorid 	= ReadPackCell(pack);
	...
	if (gi_survivor_trigger_touch[survivorid] > GetGameTime() - RTimerGetInterval(timer))
	{
		return;
	}
	
	gi_survivor_trigger			[survivorid] = 0;
	gi_survivor_trigger_touch	[survivorid] = 0.0;
...
void Event_player_attacked(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (delay <= 0.0)
	{
		if (IsValidHandle(gt_TimerThreat[survivorid]))
		{
			delete gt_TimerThreat[survivorid];
		}
	...
	gt_TimerThreat[survivorid] = CreateTimer(delay, TeleportBotToSurvivor, survivorid);
}
...
void Event_player_hurt(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (IsSurvivorIncapacitated(client) || IsSurvivorHanging(client))
	{
		...
		if (bot || IsValidHandle(gt_TimerThreat[survivorid]))
		{
			return;
		}
	}
...
{
	if (IsValidHandle(gt_BotHelps[survivorid]))
	{
		return;
	}
	...
	gt_BotHelps[survivorid] = CreateTimer(1.0, BotHelps, survivorid, TIMER_REPEAT);
}
...
bool IsAllHumansInside(int entity)
{
	...
	if (strcmp(classname, "trigger_multiple"))
	{
		entity = GetEntityTrigger(entity);
		if (!entity)
		{
			return false;
		}
...
stock int IsSurvivorReviveTarget(int client) // IsSurvivorRevive
{
...
stock int IsSurvivorReviveOwner(int client) // IsSurvivorRevived
{
...
{
	if (!IsEntityValid(entity) || (entity >= 0 && entity <= MAX_PLAYERS)) {return;}
