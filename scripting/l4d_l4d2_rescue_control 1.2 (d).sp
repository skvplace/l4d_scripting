...
public OnPluginStart()
{
	...
	gc_rescue_door_opening = CreateConVar("rescue_door_opening", "1", "Enables automatic door opening when a survivor approaches", _, true, 0.0, true, 1.0);
	SetConVarFlags(gc_rescue_door_opening, GetConVarFlags(gc_rescue_door_opening) & ~FCVAR_NOTIFY);
	
	gc_rescue_door_opening_distance = CreateConVar("rescue_door_opening_distance", "300", "Sets the automatic door opening distance", _, true, 200.0, true, 800.0);
	...
	RegAdminCmd("sm_kv", Command_kv, ADMFLAG_CONFIG, "Save gk_rooms");
}
...
public OnGameplayStart(int stage)
{
	...
	if (DEBUG)
	{
		gf_rescue_min_dead_time = 5.0;
	}
	
	PrintToChatSkv(DEBUG, "OnGameplayStart: gf_rescue_min_dead_time %f", gf_rescue_min_dead_time);
...
Action:Command_kv(client, args)
{
	KeyValues_ToFile(gk_rooms, "command");
}
	...
	for (int i = 1; i <= MAX_PLAYERS; i ++)
	{
		if (IsValidClientTeam2(i) && !IsPlayerAlive(i))
		{
			...
			if (survivorid)
			{
				...
				gt_CheckNearRescue[survivorid] = CreateTimer(gf_rescue_min_dead_time * 0.25, CheckNearRescue, survivorid);
				
				PrintToChatSkv(DEBUG, "SearchDeadSurvivor: create gt_CheckNearRescue[%d]", survivorid);
			}		
		}
	}
}
...
void Event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (survivorid)
	{
		...
		PrintToChatSkv(DEBUG, "%s: create gt_CheckNearRescue[%d]", name, survivorid);
	}
}
...
Action:CheckNearRescue(Handle timer, int survivorid)
{
	...
	PrintToChatSkv(DEBUG, "CheckNearRescue: create gt_CreateRescue[%d]", survivorid);
}

Action:CreateRescue(Handle timer, int survivorid)
{
	PrintToChatSkv(DEBUG, "CreateRescue: survivorid %d", survivorid);
	...
	//PrintToChatSkv(DEBUG_VISIBLE, "CreateRescue: IsAnySurvivorNear");
	if (IsAnySurvivorNear(room, 300.0, true))
	{
	...
	/*
	PrintToChatSkv(DEBUG, "CreateRescue: pos_spawn %d %d %d", 	RoundFloat(pos_spawn[0]), 
																	RoundFloat(pos_spawn[1]), 
																	RoundFloat(pos_spawn[2]));
																
	PrintToChatSkv(DEBUG, "CreateRescue: ang_spawn %d %d %d", 	RoundFloat(ang_spawn[0]), 
																	RoundFloat(ang_spawn[1]), 
																	RoundFloat(ang_spawn[2]));
	*/
	...
	if (GetEngineVersion() == Engine_Left4Dead)
	{
		...
		if (!pills)
		{
			...
			if (IsValidEntity(pills))
			{
				...
				PrintToChatSkv(DEBUG, "CreateRescue: spawn pills %d", pills);
			}
		}
	}
	
	PrintToChatSkv(DEBUG, "CreateRescue: spawn rescue %d", entity);
}
...
{
	PrintToChatSkv(DEBUG, "Rescue_Playsound_Timer");
	...
	if (DEBUG)
	{
		rescue_door_opening_distance = 50.0;
	}
	
	if (GetConVarInt(gc_rescue_door_opening))
	{
		int rescuer = IsAnySurvivorNear(room, rescue_door_opening_distance, false);
		if (rescuer)
		{
			OpenRoom(room, rescuer);
			return Plugin_Stop;
		}
	}
...
void OpenRoom(int room, int rescuer)
{
	if (!IsValidEntity(room))
	{
		return;
...
void Event_player_use(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	PrintToChatSkv(true, "%s: door %d, client %d", name, entity, client);
	OnOpen_Room(room, client);
}
...
void OnOpen_Door(char [] output, int door, int rescuer, float delay)
{
	...
	PrintToChatSkv(true, "OnOpen_Door: door %d, rescuer %d", door, rescuer);
	...
	while ((i = FindEntityByClassname(i, classname)) != -1)
	{
		...
		if (!StrContains(name, GLOW_NAME))
		{
			if (room == GetEntPropEnt(i, Prop_Data, "m_hOwnerEntity"))
			{
				...
				if (IsValidClientTeam2Alive(rescuer) && IsFakeClient(rescuer) && GetEngineVersion() != Engine_Left4Dead)
				{
...
stock bool IsSkyAbove(float origin[3])
{
	...
	{
		//SpawnDebugSprite(pos_start, "0 0 255");
		return true;
	}
	
	//SpawnDebugSprite(pos_start, "255 0 0");
	//PrintToChatSkv(true, "false");
	return false;
}
...
void KeyValues_ToFile(Handle kv, char [] file_name)
{
	...
	FormatEx(buffer, sizeof(buffer), "addons/sourcemod/data/RM_%s.txt", file_name);
...
{
	PrintToChatSkv(DEBUG, "GetVectorsRescue: old_room %d", old_room);
	
	float distance = GetConVarFloat(FindConVar("rescue_range"));
	
	int room = GetNearRoom(distance);
	PrintToChatSkv(DEBUG, "GetVectorsRescue: room %d", room);
	...
	if (room == old_room)
	{
		PrintToChatSkv(DEBUG, "GetVectorsRescue: room == old_room");
		return 0;
	}
	
	
	int rescue_count = GetEntProp(room, Prop_Data, "m_iHealth");
	//PrintToChatSkv(DEBUG, "GetVectorsRescue: rescue_count %d", rescue_count);
	...
	for (int i = 1; i <= rescue_count; i++)
	{
		...
		//PrintToChatSkv(DEBUG, "GetVectorsRescue: random slot %d", random);
		
		if (random <= MAX_ROOM_RESCUES)
		{
			//PrintToChatSkv(DEBUG, "GetVectorsRescue: scan slot %d", random);
			
			if (IsRescueFree(room, random, pos_spawn, ang_spawn))
			{
				PrintToChatSkv(DEBUG, "GetVectorsRescue: return %d", room);
				return room;
			}
		}
	}
	
	PrintToChatSkv(DEBUG, "GetVectorsRescue: return 0");
	return 0;
}

int GetNearRoom(float distance)
{
	PrintToChatSkv(DEBUG, "GetNearRoom: distance %f", distance);
				...
				if (GetEntProp(i, Prop_Data, "m_iHealth"))
				{
					...
					if (distance >= distance_client_i)
					{
						if (IsSameFloor(pos_client[2], pos_i[2]))
						{
							...
							else if (distance_client_i < distance_old)
							{
								distance_old = distance_client_i;
								result = i;
							}
...
bool IsRescueFree(int room, int rescue_slot, float origin[3], float angles[3])
{
	...
	PrintToChatSkv(DEBUG, "IsRescueFree: room %s", temp);
	...
	if (KvJumpToKey(gk_rooms, temp, false))
	{
		PrintToChatSkv(DEBUG, "IsRescueFree: get rescue_slot %s", temp);
		
		if (!KvGetNum(gk_rooms, "active"))
		{
			PrintToChatSkv(DEBUG, "IsRescueFree: rescue is not active");
			...
			if (strcmp(value, "none"))
			{
				...
				PrintToChatSkv(DEBUG_KV, "IsRescueFree: origin %d %d %d", 	RoundFloat(origin[0]), 
																			RoundFloat(origin[1]), 
																			RoundFloat(origin[2]));
				...
				if (strcmp(value, "none"))
				{
					...
					PrintToChatSkv(DEBUG_KV, "IsRescueFree: angles %d %d %d", 	RoundFloat(angles[0]),
																				RoundFloat(angles[1]), 
																				RoundFloat(angles[2]));
				}
			}
	...
	if (result)
	{
		...
		PrintToChatSkv(DEBUG, "IsRescueFree: update room rescue_count to %d", rescue_count);
	}
...
bool Rescue_Restore(float origin[3])
{
	...
	//PrintToChatSkv(DEBUG, "Rescue_Restore: pos_spawn %s", pos_spawn);
	...
	while ((i = FindEntityByClassname(i, "info_target")) != -1)
	{
		...
		if (!strcmp(name, ROOM_NAME))
		{
			...
			if (KvJumpToKey(gk_rooms, room, false))
			{
				...
				{
					KvGetString(gk_rooms, "origin", value, sizeof(value), "none");
					//PrintToChatSkv(DEBUG, "Rescue_Restore: origin %s", value);
					
					if (!strcmp(value, pos_spawn))
					{
						PrintToChatSkv(DEBUG, "Rescue_Restore: room %s rescue_slot %s", room, temp);
						KvSetNum(gk_rooms, "active", 0);
				...
				if (result)
				{
					...
					PrintToChatSkv(DEBUG, "Rescue_Restore: update room rescue_count to %d", rescue_count);
...
void Rescue_Restore_All()
{
	...
	//PrintToChatSkv(DEBUG, "Rescue_Restore_All");
	...
	while ((i = FindEntityByClassname(i, "info_target")) != -1)
	{
		...
		if (!strcmp(name, ROOM_NAME))
		{
			...
			if (KvJumpToKey(gk_rooms, room, false))
			{
				...
				while (KvJumpToKey(gk_rooms, temp, false))
				{
					PrintToChatSkv(DEBUG, "Rescue_Restore_All: room %s rescue_slot %s", room, temp);
					KvSetNum(gk_rooms, "active", 0);
				...
				PrintToChatSkv(DEBUG, "Rescue_Restore_All: update room rescue_count to %d", rescue_slot);
...
Action:Room_Disable(Handle timer, int room)
{
	...
	//PrintToChatSkv(DEBUG, "Room_Disable: room %d", room);
...
void Create_Recue_Rooms(int entity)
{
	...
	while ((i = FindEntityByClassname(i, "info_survivor_rescue")) != -1)
	{
		...
		if (GetVectorDistance(pos_entity, pos_i) <= RESCUE_SEARCH_DISTANCE)
		{
			if (SetRoomData(i, -1))
			{
				PrintToChatSkv(DEBUG, "Create_Recue_Rooms: create room to rescue %d", i);
			}
			else
			{
				PrintToChatSkv(DEBUG, "Create_Recue_Rooms: room not create to rescue %d", i);
			}
		}
	}
}
...
bool IsAllSurvivorFar(int entity)
{
	...
	if (!near)
	{
		PrintToChatSkv(DEBUG, "IsAllSurvivorFar: all survivors far of entity %d", entity);
		return true;
	}
...
int IsAnySurvivorNear(int entity, float distance, bool visible)
{
	float pos_entity[3];
	GetEntPropVector(entity, Prop_Send, "m_vecOrigin", pos_entity);
...
stock void SendBotToPos(int bot, float vPos[3])
{
	...
	PrintToChatSkv(DEBUG, "SendBotToPos: force bot %N", bot);
}
...
bool IsVisible(int client, int entity)
{
	...
	if (TR_DidHit(trace))
	{
		...
		if (index == entity)
		{
			//PrintToChatSkv(true, "IsVisible: index == entity, return true");
