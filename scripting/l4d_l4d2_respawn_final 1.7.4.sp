...
bool 	gb_respawn_status			[MAX_SURVIVORID + 1];
float 	gf_rescue_create_time		[MAX_SURVIVORID + 1];
float 	gf_death_time				[MAX_SURVIVORID + 1];
...
bool 	gb_rescue_spawn_inside		[MAX_SURVIVORID + 1];
...
void UseStart(char [] output, int entity, int activator, float delay)
{
	...
	for (int i = 1; i <= MAX_SURVIVORID; i++)
	{
		gb_rescue_spawn_inside[i] = result;
	}
}
...
void Event_player_death(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (survivorid)
	{
		gf_death_time[survivorid] = GetGameTime();
...
Action CreateRescue(Handle timer, int survivorid)
{
	...
	{
		gb_rescue_spawn_inside[survivorid] = true;
	}
	...
	if (!gb_rescue_spawn_inside[survivorid])
	{
	...
	gf_rescue_create_time[survivorid] = GetGameTime();
...
Action Rescue_Playsound_Timer(Handle timer, Handle h_data)
{
	...
	if (gf_respawn_outside_time)
	{
		...
		if (GetGameTime() - gf_rescue_create_time[survivorid] >= gf_respawn_outside_time && GetOriginDeadSurvivors(survivorid, pos_spawn))
		{
			gb_rescue_spawn_inside			[survivorid] = false;
			gf_rescue_create_time	[survivorid] = 0.0;
...
void Delete_Timers()
{
	...
	for (int i = 1; i <= MAX_SURVIVORID; i++)
	{
		...
		gb_rescue_spawn_inside				[i] = false;
		
		gb_respawn_status			[i] = false;
		gf_rescue_create_time		[i] = 0.0;
...
Action SearchDeadSurvivor(Handle timer)
{
	...
	for (int i = 1; i <= MAX_PLAYERS; i ++)
	{
		if (IsValidClientTeam2(i) && !IsPlayerAlive(i))
		{
			...
			if (survivorid)
			{
				death_time = GetGameTime() - gf_death_time[survivorid];
								
				if (death_time >= gf_rescue_min_dead_time_final)
				{
...
bool SetUserRespawn(int client)
{
	...
	gb_respawn_status[survivorid] = true;
...
bool IsClientRespawned(int client)
{
	...
	return gb_respawn_status[survivorid];
}
