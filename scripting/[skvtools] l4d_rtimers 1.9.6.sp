...
any native_CreateRTimer(Handle plugin, int numParams)
{
	...
	ThrowNativeError(429, "\"Attempt to create a timer, timerid %d, gh_timer %x [%s], gi_timerid_count_free %d, gi_timerid_count %d\"", i, view_as<int>(gh_timer[i]), IsValidHandle(gh_timer[i]) ? "valid":"not valid", gi_timerid_count_free, gi_timerid_count);
	return false;
}
...
int GetFreeTimerID()
{
	...
	if (gi_timerid_count_free > 0)
	{
		i = gi_timerid_free[gi_timerid_count_free];
...
any native_RTimerIsValid(Handle plugin, int numParams)
{
	int i = RTimerGetId(GetNativeCell(1));
	if (!i)
	{
...
int RTimerFire(int i, bool reset = true)
{
	if (gb_server_empty)
	{
		RTimerDelete(i);
...
void Delete_RTimers(int flags)
{
	for (int i = 1; i <= gi_timerid_count; i ++)
	{
		...
		else if (gi_timer_flags[i] & flags)
		{
			RTimerDelete(i);
		}
	}
	
	gi_timerid_count_free = 0;
...
void RTimerClearId(int i, bool count_available = true)
{
	...
	gh_plugin			[i] = null;
	gh_func				[i] = INVALID_FUNCTION;
	gh_func_close 		[i] = INVALID_FUNCTION;
