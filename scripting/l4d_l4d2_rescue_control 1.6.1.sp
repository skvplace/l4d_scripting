	...
	for (int i = 1; i <= MAX_PLAYERS; i ++)
	{
		if (IsValidClientTeam2(i) && !IsPlayerAlive(i))
		{
			...
			if (survivorid)
			{
				if (!IsValidHandle(gt_CheckNearRescue[survivorid]))
				{
					gt_CheckNearRescue[survivorid] = RTimerCreate((gf_rescue_min_dead_time * 0.25) + (i * 0.5), CheckNearRescue, survivorid);
				}
			}		
		}
	}
...
public void Event_player_death(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (survivorid)
	{
		if (!IsValidHandle(gt_CheckNearRescue[survivorid]))
		{
			gt_CheckNearRescue[survivorid] = RTimerCreate(gf_rescue_min_dead_time, CheckNearRescue, survivorid);
		}
	}
}
