...
any native_CreateRTimer(Handle plugin, int numParams)
{
	...
	if (!IsRtimerSpawn())
	{
		if (interval < 0.04 || (gametime < 1.21 && gametime + interval <= 1.21 + MIN_INTERVAL)) // 1.20 - время спавна logic_timer
		{
	...
	ServerCommand("sm_dump_handles addons/sourcemod/logs/error_rtimer.txt");
...
void OnTimer(char [] output, int timer, int activator, float delay)
{
	...
	for (int i = 1; i <= MAX_RTIMERS; i++)
	{
		if (gh_timer[i] && gf_timer_firetime[i] > 0.0)
		{
...
{
	if (IsValidPlugin(gh_plugin[i]) && gh_func_close[i])
	{
...
void Transfer_RTimers()
{
	...
	for (int i = 1; i <= MAX_RTIMERS; i++)
	{
		if (gh_timer[i] && gf_timer_firetime[i] > gametime)
		{
