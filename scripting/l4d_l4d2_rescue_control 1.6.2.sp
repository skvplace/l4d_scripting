	...
	for (int i = 1; i <= MAX_PLAYERS; i ++)
	{
		if (IsValidClientTeam2(i) && !IsPlayerAlive(i))
		{
			...
			if (survivorid)
			{
				...
				time_respawn = gf_rescue_min_dead_time - death_time;
				...
				if (!IsValidHandle(gt_CheckNearRescue[survivorid]))
				{
					gt_CheckNearRescue[survivorid] = RTimerCreate(time_respawn + (i * 0.5), CheckNearRescue, survivorid, TIMER_FLAG_NO_ROUNDCHANGE);
				}
			}		
		}
	}
