...
Handle 	gt_CheckNearRescue			[MAX_SURVIVORID + 1];
	...
	if (gi_rescue_mode == 0)
	{
		...
		for (int i = 1; i <= MAX_PLAYERS; i++)
		{
			...
			if (survivorid)
			{
				...
				{
					RTimerKill(gt_CheckNearRescue[survivorid]);
				}
	...
	for (int i = 1; i <= MAX_PLAYERS; i ++)
	{
		if (IsValidClientTeam2(i) && !IsPlayerAlive(i))
		{
			...
			if (survivorid)
			{
				if (!IsValidHandle(gt_CheckNearRescue[survivorid]))
				{
					gt_CheckNearRescue[survivorid] = RTimerCreate((gf_rescue_min_dead_time * 0.25) + (i * 0.5), CheckNearRescue, survivorid, TIMER_FLAG_NO_ROUNDCHANGE);
				}
			}		
		}
	}
...
public void Event_player_team(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (!IsValidHandle(gt_CheckNearRescue[survivorid]) &&
		!IsValidHandle(gt_CreateRescue[survivorid]))
	{
		gt_CheckNearRescue[survivorid] = RTimerCreate(gf_rescue_min_dead_time, CheckNearRescue, survivorid, TIMER_FLAG_NO_ROUNDCHANGE);
	}
}
...
void CheckNearRescue(Handle timer, int survivorid)
{
	if (gb_finale_start)
	{
		return;
...
void Delete_Timers()
{
	...
	for (int i = 1; i <= MAX_SURVIVORID; i++)
	{
		...
		{
			RTimerKill(gt_CheckNearRescue[i]);
		}
