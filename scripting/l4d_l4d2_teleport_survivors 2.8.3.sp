...
public OnPluginStart()
{
	...
	gc_teleport_bot_threat_distance	= CreateConVar("ts_teleport_bot_threat_distance", "400", "If the bots are further than this distance, they will be moved", _, true, 300.0, true, 2000.0); 
...
void Event_player_attacked(Handle:event, const String:name[], bool:dontBroadcast)
{
...
void Event_player_released(Handle:event, const String:name[], bool:dontBroadcast)
{
	int victim = GetClientOfUserId(GetEventInt(event, "victim"));
	if (!IsValidClientTeam2Alive(victim)) {return;}
...
void Event_player_hurt(Handle:event, const String:name[], bool:dontBroadcast)
{
...
void Event_player_need_help(Handle:event, const String:name[], bool:dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!IsValidClientTeam2Alive(client)) {return;}
...
void Event_revive_success(Handle:event, const String:name[], bool:dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "userid"));
	if (!IsValidClientTeam2Alive(client)) {return;}
...
Action:TeleportBotToSurvivor(Handle timer, int survivorid)
{
