...
void Event_player_death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (survivorid)
	{
		...
		{
			if (gi_final_rescue_slot_client[i] == survivorid)
			{
...
Action:CreateRescue(Handle timer, int survivorid)
{
	...
	{
		if (!GetVectorsFinalRescue(survivorid, pos_spawn, ang_spawn))
		{
...
void Event_player_use(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else if (!strcmp(classname, "weapon_pain_pills_spawn") || !strcmp(classname, "weapon_pain_pills")) 
	{
		...
		if (StrContains(targetname, GLOW_NAME) > -1)
		{
			...
			if (IsValidClient(client))
			{
				int prop = GetClientActiveProp(client);
				if (prop)
				{
					int rescue = GetEntPropEnt(prop, Prop_Data, "m_hOwnerEntity");
					//PrintToChatAll("%s: rescue %d", name, rescue);
...
bool OpenDoor(int rescue, int rescuer)
{
	...
	Respawn(GetClientOfNameEntity(rescue), rescuer, pos_spawn, ang_spawn);
	...
	for (int i = 1; i <= MAX_PLAYERS; i ++)
	{
		if (IsValidClientTeam2(i) && !IsPlayerAlive(i))
		{
			...
			if (survivorid)
			{
				...
				gt_CheckClientDead[survivorid] = CreateTimer(time_respawn, CheckClientDead, survivorid);
			}		
		}
	}
}
...
bool SetNearRescueVectors(int entity)
{
	...
	for (int j = 1; j <= MAX_FINAL_RESCUES; j ++)
	{
		...
		if (near_rescue_slot)
		{
			...
			if (slot)
			{
				//PrintToChatAll("SetNearRescueVectors: save near_rescue_slot %d", near_rescue_slot);
...
bool GetVectorsFinalRescue(int survivorid, float pos_spawn[3], float ang_spawn[3])
{
	//PrintToChatAll("GetVectorsFinalRescue: client %N, pos_spawn %d %d %d", GetClientOfSurvivorId(survivorid), 	RoundFloat(pos_spawn[0]), RoundFloat(pos_spawn[1]), RoundFloat(pos_spawn[2]));
	...
	for (int i = 1; i <= MAX_FINAL_RESCUES; i++)
	{
		if (gi_final_rescue_slot_client[i] == survivorid)
		{
			if (!pos_spawn[0] && !pos_spawn[1] && !pos_spawn[2])
			{
				pos_spawn = gv_rescue_origin[gi_final_rescue_slot[i]];
				ang_spawn = gv_rescue_angles[gi_final_rescue_slot[i]];
				
				//PrintToChatAll("GetVectorsFinalRescue: update NULL origin true");
				return true;
			}
			
			//PrintToChatAll("GetVectorsFinalRescue: false 1");
			return false;
		}
	}
	...
	for (int i = 1; i <= MAX_FINAL_RESCUES; i++)
	{
		...
		if (random <= MAX_FINAL_RESCUES)
		{
			...
			{
				gi_final_rescue_slot_client[random] = survivorid; // активная респа
					
				pos_spawn = gv_rescue_origin[gi_final_rescue_slot[random]];
				ang_spawn = gv_rescue_angles[gi_final_rescue_slot[random]];
					
				//PrintToChatAll("GetVectorsFinalRescue: set active random slot %d, client %N", random, GetClientOfSurvivorId(survivorid));
	...
	//PrintToChatAll("GetVectorsFinalRescue: 2");
	/*
	for (int i = 1; i <= MAX_FINAL_RESCUES; i++)
	{
		if (gi_final_rescue_slot[i] && !gi_final_rescue_slot_client[i])
		{
			gi_final_rescue_slot_client[i] = survivorid; // активная респа
			
			pos_spawn = gv_rescue_origin[gi_final_rescue_slot[i]];
			ang_spawn = gv_rescue_angles[gi_final_rescue_slot[i]];
			
			//PrintToChatAll("GetVectorsFinalRescue: 2 return true");
			
			return true;
		}
	}
	*/
	//PrintToChatAll("GetVectorsFinalRescue: return false 2");
	return false;
}
