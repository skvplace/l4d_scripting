...
{
	PrintToConsole(client, "# survivorid name userid clientid");
	
	for (int i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			PrintToConsole(client, "# %d \"%N\" %d %d %s", GetClientSurvivorId(i), 
															i,
															GetClientUserId(i),
															i,
															IsFakeClient(i) ? "BOT":"");
...
//бот заменил игрока
void Event_player_bot_replace(Handle event, const char[] name, bool dontBroadcast)
{
...
//игрок заменил бота
void Event_bot_player_replace(Handle event, const char[] name, bool dontBroadcast)
{
	int client = GetClientOfUserId(GetEventInt(event, "player"));
	if (!IsValidClient(client)) {return;}
