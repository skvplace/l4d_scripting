...
void OnConVarChanged_rescue_mode(ConVar convar, const char[] oldValue, const char[] newValue)
{
	...
	LogToFileEx(gs_logpath, "gi_rescue_mode change to %d", gi_rescue_mode);
...
void Event_create_panic_event(Handle event, const char[] name, bool dontBroadcast)
{
	...
	LogToFileEx(gs_logpath, "create_panic_event: force gc_rescue_mode 0");
...
{
	LogToFileEx(gs_logpath, "RescueMode_Restore: force mode to %d", gi_rescue_mode_restore);
