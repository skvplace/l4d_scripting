...
Action Teleport(Handle timer, int ref)
{
	...
	if (strcmp(classname, "trigger_finale"))
	{
		...
		for (int i = 1; i <= MAX_PLAYERS; i++)
		{
			if (IsValidClientTeam3Alive(i) && IsClientInTrigger(i, trigger))
			{
				...
				{
					RTimerKill(gt_Teleport);
					gt_Teleport = RTimerCreate(1.0, Teleport, ref, TIMER_REPEAT);
				}
